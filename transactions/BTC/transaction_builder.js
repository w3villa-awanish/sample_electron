...
const baddress = require('../../../node_modules/bitcoinjs-lib/src/address');
...
class TransactionBuilder {
 ...
 ChaKeySign(
    signParams, Name_Index, bShowNumer, CB,
    redeemScript,
    hashType,
    witnessValue,
    witnessScript,
  ) {
   return  ChagetSigningData(
      this.network,
      this.__INPUTS,
      this.__needsOutputs.bind(this),
      this.__TX,
      signParams,Name_Index, bShowNumer, 
      redeemScript,
      hashType,
      witnessValue,
      witnessScript,
...
function ChagetSigningData(
  network,
  inputs,
  needsOutputs,
  tx,
  signParams,
  Name_Index, bShowNumer, 
  redeemScript,
  hashType,
  witnessValue,
  witnessScript,
  ...
  {
    ourPubKey=ServerChaKey.mIChaKey.ExportPubKeyByIndex(Name_Index);
  ...
  {
    ourPubKey=ServerChaKey.mIChaKey.ExportPubKeyByTokensName(Name_Index);
    lasterror=ServerChaKey.mIChaKey.GetLastError();
    if(lasterror!=0)return lasterror;
  ...
  // ready to sign
  let signatureHash;
  ServerChaKey.mIChaKey.SetChaSignFlag(true);
  if (input.hasWitness) {
    signatureHash = tx.hashForWitnessV0(
      vin,
      input.signScript,
  ...
  lasterror=ServerChaKey.mIChaKey.GetLastError();
  if(lasterror!=IChaKey.SIGN_ONLY_NOT_WAITFOR_USER)
  {
    if(lasterror!=0)return lasterror;
    ServerChaKey.mIChaKey.SetChaSignFlag(false);
    lasterror=ServerChaKey.mIChaKey.GetLastError();
    if(lasterror!=0)return lasterror;
    ...
    var   gPaystatus=ServerChaKey.mIChaKey.GetLastHashData()
    var  signature=gPaystatus.HashData.slice(1,65)
    ChatrySign(input,ourPubKey,signature,hashType)
  }
  return lasterror;
...
function WaitforSign(input,hashType,ourPubKey,CB)
{
    ...
    if (lasterror!=0 && lasterror!=IChaKey.SIGN_ONLY_NOT_WAITFOR_USER) 
    {
    ...
    var  signature=Paystatus.HashData.slice(1,65)
    ChatrySign(input,ourPubKey,signature,hashType)
    CB(lasterror,Paystatus);
}
